...
class HelpTopicController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->lang as $index => $key) {
            if ($request->question[$index] && $key != 'en') {
                Translation::updateOrInsert(
                    [
                        'translationable_type' => 'App\Model\HelpTopic',
                        'translationable_id' => $helps->id,
                        'locale' => $key,
                        'key' => 'question'
                    ],
                    [
                        'value' => $request->question[$index]
                    ]
                );
            }else if($request->question[$index] && $key == 'en'){
                $helps->question = $request->question[$index];
            }
            if ($request->answer[$index] && $key != 'en') {
                Translation::updateOrInsert(
                    [
                        'translationable_type' => 'App\Model\HelpTopic',
                        'translationable_id' => $helps->id,
                    ...
                    [
                        'value' => $request->answer[$index]
                    ]
                );
            }else if($request->answer[$index] && $key == 'en'){
                $helps->answer = $request->answer[$index];
            }
        }
    ...
    {
        $helps = HelpTopic::with('translations')->findOrFail($id);

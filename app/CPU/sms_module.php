...
class SMS_module
{
    ...
    {
        $response = self::etisalat($receiver, $otp);
        return $response;
    ...
    public static function etisalat($receiver, $otp)
    {
        $res = 'error';
        $user = "technologiesadmin";
        $pass = "@Tech2k22";
        $curl = curl_init();
        $post = array(
            'username' => $user,
        ...
        $post = array(
            ...
            'contentType' => '3.1',
            'priority' => '1',
            'recipient' => str_replace("+", "", $receiver),
            'expiryDt' => Carbon::now()->addDays(10),
            'dr' => '1',
            'senderAddr' => 'HRF',
            'dndCategory' => 'Campaign',
            'msg' => 'You OTP Code is: ' . $otp
        );
        $post = json_encode($post);
        curl_setopt_array($curl, array(

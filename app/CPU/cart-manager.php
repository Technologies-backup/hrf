...
class CartManager
{
    ...
    public static function get_shipping_title($group_id = null)
    {
        if ($group_id == null) {
            $title = CartShipping::whereIn('cart_group_id', CartManager::get_cart_group_ids())->first()->shipping_method->title;
        } else {
            $data = CartShipping::where('cart_group_id', $group_id)->first();
            $title = isset($data) ? $data->shipping_method->title : "";
        }
        return $title;
    }

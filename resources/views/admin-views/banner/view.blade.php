    ...
    <div class="content container-fluid">
        ...
        <div class="row">
            <div class="col-md-12" id="banner-btn">
                ...
                <button id="video-banner-add" class="btn btn-primary"><i
                        class="tio-add-circle"></i> {{ \App\CPU\translate('add_video_banner')}}</button>
        ...
        <div class="row pt-4" id="video-banner" style="display: none;text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        {{ \App\CPU\translate('video_banner')}}
                    </div>
                    <div class="card-body">
                        <form action="{{route('admin.banner.store')}}" method="post" enctype="multipart/form-data"
                              class="banner_form">
                            ...
                            <div class="form-group">
                                <div class="row">
                                    <input type="hidden" id="id" name="id">
                                    <input type="hidden" id="video_type" name="banner_type" value="Video Banner">
                                    @foreach(json_decode($language) as $lang)
                                    <div class="col-md-6">
                                        <label for="name">{{ \App\CPU\translate('Video').'('.strtoupper($lang).')'}}</label><span
                                                class="badge badge-soft-danger">( {{\App\CPU\translate('ratio')}} 4:1 )</span>
                                        <br>
                                        <div class="custom-file" style="text-align: left">
                                            <input type="file" name="url[]" id="{{$lang}}_vbvideoFileUploader"
                                                class="custom-file-input"
                                                accept="video/mp4,video/*">
                                            <label class="custom-file-label"
                                                for="{{$lang}}_vbvideoFileUploader">{{\App\CPU\translate('choose')}} {{\App\CPU\translate('file')}}</label>
                                        </div>
                                        <label for="name">{{ \App\CPU\translate('Thumbnail').'('.strtoupper($lang).')'}}</label><span
                                            class="badge badge-soft-danger">( {{\App\CPU\translate('ratio')}} 4:1 )</span>
                                        <br>
                                        <div class="custom-file" style="text-align: left">
                                            <input type="file" name="image[]" id="{{$lang}}_vbimageFileUploader"
                                                class="custom-file-input vbimageFileUploader"
                                                accept=".jpg, .png, .jpeg, .gif, .bmp, .tif, .tiff|image/*">
                                            <label class="custom-file-label"
                                                for="{{$lang}}_vbimageFileUploader">{{\App\CPU\translate('choose')}} {{\App\CPU\translate('file')}}</label>
                                        </div>
...
@push('script')
    ...
    <script>
        ...
        $(".vbimageFileUploader").change(function () {
            id = $(this).attr('id').split('_')[0];
            vbimagereadURL(this, id);
        });

        function vbimagereadURL(input, id) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();

                reader.onload = function (e) {
                    $('#'+id+'_vbImageviewer').attr('src', e.target.result);
                }
    ...
    <script>
        ...
        $('#video-banner-add').on('click', function () {
            $('#video-banner').show();
            $('#main-banner').hide();
            $('#secondary-banner').hide();
            $('#popup-banner').hide();
        ...
        $(document).on('click', '.edit', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.banner_type == 'Main Banner') {
                    ...
                    } else if (data.banner_type == 'Video Banner') {
                        ...
                        $.each(data.translations, function(key, value){
                            // $('#'+value.locale+'_'+value.key).val(value.value);
                            if(key == "photo"){
                                $('#'+value.locale+'_vbImageviewer').attr('src', "{{asset('storage/app/public/banner')}}" + "/" + value.value);
                            }
                        })
                        // $('#en_url').val(data.url);
                        $('#video_type').siblings('#id').val(data.id);
                        $('#en_vbImageviewer').attr('src', "{{asset('storage/app/public/banner')}}" + "/" + data.photo);
                        $('#cate-table').hide();
